...
public class DialogRuntime {
    ...
    public void start() {
        broadcast(dialogMessage("message.chatdialogs.dialog.started", definition.displayName()));
    ...
    public void stop() {
        ...
        active = false;
        broadcast(dialogMessage("message.chatdialogs.dialog.stopped", definition.displayName()));
    ...
    public void choose(String token) {
        ...
        if (buttonId == null) {
            player.sendSystemMessage(dialogMessage("message.chatdialogs.dialog.button.inactive"));
            return;
        }
        DialogButton button = buttonsById.get(buttonId);
        if (button == null || usedButtons.contains(button.id())) {
            player.sendSystemMessage(dialogMessage("message.chatdialogs.dialog.button.used"));
            return;
        }
        DialogContext context = context(Map.of());
        boolean allowed = button.conditions().stream().allMatch(condition -> condition.test(context));
        if (!allowed) {
            player.sendSystemMessage(dialogMessage("message.chatdialogs.dialog.button.conditions"));
            return;
        }
    ...
    private void executeButton(DialogButton button) {
        ...
        for (DialogAction action : button.actions()) {
            try {
                ...
                ChatDialogs.LOGGER.error("Failed to execute button action", e);
            }
        }
    ...
    private void enterNode(String nodeId) {
        ...
        try {
            ...
            ChatDialogs.LOGGER.warn("Missing dialog node {} in dialog {}", nodeId, definition.id());
            stop();
            return;
        }
        ...
        for (DialogAction action : node.entryActions()) {
            try {
                ...
                ChatDialogs.LOGGER.error("Failed to execute node action", e);
            }
        }
            ...
            if (result) {
                for (DialogAction action : branch.actions()) {
                    try {
                        ...
                        ChatDialogs.LOGGER.error("Failed to execute branch action", e);
                    }
                }
    ...
    private void sendButtons(DialogNode node) {
        ...
        for (DialogButton button : node.buttons()) {
            ...
            clickable.withStyle(style -> style
                    ...
                    .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.translatable("message.chatdialogs.dialog.button.hover")))
                    .withUnderlined(true));
    ...
    private Component dialogMessage(String translationKey, Object... args) {
        MutableComponent prefix = Component.translatable("message.chatdialogs.dialog.prefix").withStyle(ChatFormatting.GOLD);
